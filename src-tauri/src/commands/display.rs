...
/// Returns the current live stacking accumulation buffer (running mean) as a
/// display-resolution JPEG data URL, auto-scaled to the buffer's actual pixel
/// range. Returns Ok(None) if no live stack is active. Same downsample +
/// auto-scale approach as get_stack_frame, extended for the 3-channel case
/// since live stack frames are always debayered before accumulation.
#[tauri::command]
pub fn get_live_stack_frame(state: State<Arc<PhotoxState>>) -> Result<Option<String>, String> {
    let ctx = state.context.lock().expect("context lock poisoned");

    let live_stack = match &ctx.live_stack {
        Some(ls) => ls,
        None => return Ok(None),
    };

    let pixels   = live_stack.mean_as_f32();
    let src_w    = live_stack.width;
    let src_h    = live_stack.height;
    let channels = live_stack.channels;
    ...
    // Find the actual pixel range for auto-scaling (sample the luminance
    // channel for RGB rather than every channel independently, to keep a
    // consistent stretch across channels).
    let sample_stride = if channels == 3 { 3 } else { 1 };
    let max_val = pixels.iter().step_by(sample_stride)
        .filter(|v| v.is_finite())
        .cloned()
        .fold(f32::NEG_INFINITY, f32::max);
    let min_val = pixels.iter().step_by(sample_stride)
        .filter(|v| v.is_finite())
        .cloned()
        .fold(f32::INFINITY, f32::min);
    let range = (max_val - min_val).max(1e-6);
            ...
            if channels == 3 {
                ...
                for dy in 0..step {
                    ...
                    for dx in 0..step {
                        ...
                        if sx >= src_w { continue; }
                        let idx = (sy * src_w + sx) * 3;
                        if pixels[idx].is_finite() {
                            sr += pixels[idx]; sg += pixels[idx + 1]; sb += pixels[idx + 2]; sc += 1;
                        }
                    }
                }
                if sc > 0 {
                    let sc = sc as f32;
                    rgb.push((((sr / sc) - min_val) / range * 255.0).clamp(0.0, 255.0) as u8);
                    rgb.push((((sg / sc) - min_val) / range * 255.0).clamp(0.0, 255.0) as u8);
                    rgb.push((((sb / sc) - min_val) / range * 255.0).clamp(0.0, 255.0) as u8);
                } else {
                    rgb.push(0); rgb.push(0); rgb.push(0);
                }
            } else {
                let mut sum = 0.0f32; let mut count = 0u32;
                for dy in 0..step {
                ...
                rgb.push(scaled); rgb.push(scaled); rgb.push(scaled);
            }
        }
    }

    let img = image::RgbImage::from_raw(disp_w as u32, disp_h as u32, rgb)
        .ok_or_else(|| "Failed to create live stack preview image".to_string())?;
    let mut buf = std::io::Cursor::new(Vec::new());
    image::codecs::jpeg::JpegEncoder::new_with_quality(&mut buf, 90)
        .encode_image(&img)
        .map_err(|e| e.to_string())?;
