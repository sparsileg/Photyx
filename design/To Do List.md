...
9. ~~**Enhance AnalyzeFrames:**~~
    ...
    - ~~File menu restructured — Read Single Image + Exit only~~
      ...
      ~~- Commit enhancement — move rejected files to `rejected/` subfolder with `.rejected` appended and write PXFLAG to files. Disabled on IMPORTED data.~~
      ~~- Toggle PXFLAG value in Results Table and be able to save the result, if desired, when the Commit button is pressed~~
      ~~- Test Commit from graph~~
      ~~- Does it close the image, the view, and the session?~~
      ~~- Do images manually set to pass NOT get moved to rejected?~~
      14- **Deferred:**
    - Memory audit
