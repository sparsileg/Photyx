...
| Phase 10    | In progress | - Async dispatch (long-running commands block UI; requires Tauri event system)<br/>- AnalyzeFrames progress reporting (requires async dispatch)<br/>- AnalyzeFrames standalone CLI binary / external API access / REST API to enable batch processing<br/>- Based on analysis, set default parameters for both broad and narrow band image sessions. |
